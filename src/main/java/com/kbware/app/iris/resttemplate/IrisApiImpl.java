...
public class IrisApiImpl extends RestTemplateInvoke implements IrisRestClient {
  ...
  public ResponseEntity<IrisResponseDto> updateEsimProfile(IrisDto dto) {
    RestRequest restRequest = RestRequest.builder().requestEntity(new HttpEntity<>(dto, createHeaders()))
      .url(irisUrl + UPDATE_PROFILE).httpMethod(HttpMethod.POST).responseClazz(IrisResponseDto.class)
      .uriVariables(new Object[]{}).build();
