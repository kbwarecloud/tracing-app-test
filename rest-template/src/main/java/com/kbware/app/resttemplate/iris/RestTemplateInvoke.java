...
public abstract class RestTemplateInvoke {
  ...
  private <T> ResponseEntity<T> exchange(RestRequest restRequest) {
    if (restRequest.getResponseClazz() != null) {
      return restTemplate.exchange(
          ...
          restRequest.getRequestEntity(),
          restRequest.getResponseClazz(),
          restRequest.getUriVariables());
    } else {
      return restTemplate.exchange(
          restRequest.getUrl(),
          restRequest.getHttpMethod(),
  ...
  private void logRequest(RestRequest restRequest) {
    try {
      log.debug(
          String.format(
              "Call [%s] %s with params:%s, requestEntity: %s",
              restRequest.getHttpMethod(),
              restRequest.getUrl(),
              Arrays.toString(restRequest.getUriVariables()),
              objectMapper.writeValueAsString(restRequest.getRequestEntity())));
    } catch (JsonProcessingException e) {
      log.debug(
          String.format(
              "Call [%s] %s with params:%s, requestHeaders: %s",
              restRequest.getHttpMethod(),
              restRequest.getUrl(),
              Arrays.toString(restRequest.getUriVariables()),
              restRequest.getRequestEntity().getHeaders()));
    }
  }
  ...
  private <T> void logResponse(RestRequest restRequest, ResponseEntity<T> response) {
    try {
      log.debug(
          String.format(
              "Response from call [%s] %s with params:%s, Response status: %s, responseHeaders: %s, responseBody: %s",
              restRequest.getHttpMethod(),
              restRequest.getUrl(),
              Arrays.toString(restRequest.getUriVariables()),
  ...
  public String createErrorMessage(
      String responseBody, String errorMessage, RestRequest restRequest) {
    var defaultMessage =
        String.format(
            "URI: %s, UriVariables: %s, Message: %s, ResponseBody: {%s}",
            restRequest.getUrl(),
            Arrays.toString(restRequest.getUriVariables()),
            errorMessage,
            responseBody);
